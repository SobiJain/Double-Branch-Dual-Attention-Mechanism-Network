...
class SSGC_bandasfirstdim(nn.Module):
    def __init__(self, band, classes):
        super(SSGC_bandasfirstdim, self).__init__()

        self.name = "SSGC_bandasfirstdim"
        self.r = 2
        # spectral branch
        ...
        self.conv15 = nn.Conv3d(in_channels=60, out_channels=60, padding = "valid",
                                kernel_size=(49, 1, 1), stride=(1, 1, 1)) # kernel size随数据变化
        ...
        self.conv25 = nn.Conv2d(121, 121//self.r,
                    kernel_size=(1, 1), stride=(1,1))

        self.layer_norm21 = nn.Sequential(
                                    nn.LayerNorm(121//self.r, eps=0.001, elementwise_affine=True),
                                    nn.ReLU(inplace=True)
        )

        self.conv26 = nn.Conv2d(121//self.r, 121,
                    kernel_size=(1, 1), stride=(1,1))
    ...
    def forward(self, X):
        ...
        X = X.reshape(X.shape[0], 1, 103, 11, 11)
        # spectral
        x11 = self.conv11(X)
        # print('x11', x11.shape)
        x12 = self.batch_norm11(x11)
        ...
        x16 = self.conv15(x16)
        # print('x16', x16.shape)  # 7*7*97, 60
        x16 = x16.reshape(x16.shape[0], 60, 11, 11)
        # print('x16', x16.shape)
        ...
        #multiplying both branches
        x19 = torch.einsum("bik,bkjl->bijl", x18, x17)
        # print('x19', x19.shape)
        ...
        x25 = torch.cat((x21, x22, x23, x24), dim=1)
        # print('x25', x25.shape)
        x25 = x25.reshape(x25.shape[0], 60, 11, 11)
        # print('x25', x25.shape)
        ...
        #subbranch 2
        x27 = x25.reshape(x25.shape[0], x25.shape[3]*x25.shape[2], 60)
        # print('x27', x27.shape)

        #mat mul of both branches
        x28 = torch.einsum("bik,bkjl->bijl", x27, x26)
        x28 = self.conv25(x28)
        # print('x28', x28.shape)
        x28 = x28.view(x28.shape[0], x28.shape[1])
        x28 = self.layer_norm21(x28)
        ...
        x29 = x28.reshape(x28.shape[0], 1, 11, 11)
        ...
        x41 = torch.cat((x30, x20), dim = 1)
        # print('x41', x41.shape)
        output = self.full_connection(x41.squeeze(-1))
        # print(output.shape)
